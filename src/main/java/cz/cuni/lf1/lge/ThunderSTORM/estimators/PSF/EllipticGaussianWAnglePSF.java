...
public class EllipticGaussianWAnglePSF extends PSFModel {
    ...
    public double getValue(double[] params, double x, double y) {
        double sinfi = MathProxy.sin(params[Params.ANGLE]);
        double cosfi = MathProxy.cos(params[Params.ANGLE]);
        double dx = ((x - params[Params.X]) * cosfi - (y - params[Params.Y]) * sinfi);
        double dy = ((x - params[Params.X]) * sinfi + (y - params[Params.Y]) * cosfi);

        return params[Params.INTENSITY] / (2 * MathProxy.PI * params[Params.SIGMA1] * params[Params.SIGMA2]) * MathProxy.exp(-0.5 * (MathProxy.sqr(dx / params[Params.SIGMA1]) + MathProxy.sqr(dy / params[Params.SIGMA2]))) + params[Params.OFFSET];
    }
    ...
    public MultivariateMatrixFunction getJacobianFunction(final double[] xgrid, final double[] ygrid) {
        return new MultivariateMatrixFunction() {
            ...
            public double[][] value(double[] point) throws IllegalArgumentException {
                ...
                double sinfi = MathProxy.sin(transformedPoint[Params.ANGLE]);
                double cosfi = MathProxy.cos(transformedPoint[Params.ANGLE]);
                ...
                for (int i = 0; i < xgrid.length; i++) {
                    ...
                    double expVal = MathProxy.exp(-0.5 * (MathProxy.sqr(first) / sigma1Squared + MathProxy.sqr(second) / sigma2Squared));
                    double oneDivPISS2 = 1 / (MathProxy.PI * transformedPoint[Params.SIGMA1] * transformedPoint[Params.SIGMA2]);
                    //d()/dx
                    ...
                    //d()/dsigma1
                    retVal[i][Params.SIGMA1] = transformedPoint[Params.INTENSITY] * expVal * oneDivPISS2 / point[Params.SIGMA1] * (-1 + MathProxy.sqr(first) / sigma1Squared);
                    //d()/dsigma2
                    retVal[i][Params.SIGMA2] = transformedPoint[Params.INTENSITY] * expVal * oneDivPISS2 / point[Params.SIGMA2] * (-1 + MathProxy.sqr(second) / sigma2Squared);
                    //d()/dbkg
